...
namespace Framework
{
    ...
    private:
		...
		map<unsigned, map<unsigned, MLContinuous::MLAgent *>> transferedAgents;

		MLContinuous::INodeBrain *nodeBrain;
		MLContinuous::IMaze *maze;

		bool shareInfo;
		float slidingWindow;
		float minRadiusOfCurv;
		float minTurningAngle;
		unsigned immediateMoves;
		unsigned runNumber;

    public:
		MLNode(unsigned rank, bool allowSharing, float slidingWindow, float minRadiusOfCurv, float minTurningAngle, unsigned immediateMoves, unsigned runNumber);
		MLNode(unsigned rank, bool allowSharing, float slidingWindow, float minRadiusOfCurv, float minTurningAngle, unsigned immediateMoves, unsigned runNumber, Agents::IAgent *newAgent);
        virtual ~MLNode();
