...
using namespace MLContinuous;

namespace Framework
{
	MLNode::MLNode(unsigned rank, bool allowSharing, float slidingWindow, float minRadiusOfCurv, float minTurningAngle, unsigned immediateMoves, unsigned runNumber) 
		: Node(rank, allowSharing), nodeBrain(NULL), slidingWindow(slidingWindow), minRadiusOfCurv(minRadiusOfCurv), minTurningAngle(minTurningAngle), immediateMoves(immediateMoves), runNumber(runNumber)
	{
		Init();
	}

    MLNode::MLNode(unsigned rank, bool allowSharing, float slidingWindow, float minRadiusOfCurv, float minTurningAngle, unsigned immediateMoves, unsigned runNumber, IAgent *newAgent) 
		: Node(rank, allowSharing, newAgent), nodeBrain(NULL), runNumber(runNumber)
    {
		Init();
    }
	...
	{	
		...
		if (pThis != NULL)
		{
			...
			if ((mazeAgentPackage != NULL)&&(pThis->maze != NULL)&&(pThis->nodeBrain != NULL))
			{
				...
				MLAgent *agent = new MLAgent(pThis->myRank, 
											 0, 
											 "", 
											 ySize, 
											 xSize, 
											 pThis->maze, 
											 pThis->nodeBrain->GetBrain(),
											 pThis->minRadiusOfCurv,
											 pThis->minTurningAngle,
											 pThis->slidingWindow,
											 pThis->immediateMoves,
											 pThis->runNumber,
											 false, 
											 false);
		...
		if (pThis != NULL)
		{
			...
			if ((mazeAgentPackage != NULL)&&(pThis->maze != NULL)&&(pThis->nodeBrain != NULL))
			{
				...
				{					
					...
					string agentFile = "Out\\transfer" + nodeID + "_" + agentID;
					...
					pThis->transferedAgents[agentInfo.agentId][agentInfo.transferId] = agent;
				...
				{
					if (pThis->transferedAgents[agentInfo.agentId].find(agentInfo.transferId) == pThis->transferedAgents[agentInfo.agentId].end())
					{
						pthread_t *workerThread = new pthread_t();	
						...
						out << agentInfo.transferId;
						transferID = out.str();

						out.str("");
						out.flush();

						string agentFile = "Out\\transfer" + nodeID + "_" + agentID + "_" + transferID;
						char *fileName = new char[FILE_NAME_SIZE];
					...
					{
						if (pThis->transferedAgents[agentInfo.agentId][agentInfo.transferId] != NULL)
						{
							pThis->transferedAgents[agentInfo.agentId][agentInfo.transferId]->SetAgentInfo(agentInfo);
							pThis->transferedAgents[agentInfo.agentId][agentInfo.transferId]->SignalAgentTransfered();
						}
					}
				}
			}
	...
	/*static*/ void *MLNode::NewAgentThread(void *arg)
	{
		...
		if (!Validator::IsNull(agent, NAME("agent")))
		{
			...
			string agentFile = "Out\\out" + nodeID + "_" + agentID;
			char *fileName = new char[FILE_NAME_SIZE];
