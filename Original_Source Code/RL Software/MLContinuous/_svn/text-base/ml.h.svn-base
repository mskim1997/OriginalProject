...
typedef struct
{
	...
	float minRadiusOfCurvature;		// Minimum radius of curvature that must be satisfy by the agent's path.
	float slidingWindowLookAhead;	// The agent's sliding window look ahead.  This value is used to figure out the next sub-goal.
	unsigned immediateMoves;		// The n number of moves the agent has to make before recalculating its path.
	float absalonSquared;			// Arbitrary constant.
	float gradientStepSize;			// The step constant to multiply the gradient by.
	float minTurningAngle;			// Minimum turning angle that must be satisfy by the agent's turning.
} AgentInfo;
