...
namespace MLContinuous
{
  ...
  case CHECK_LIMIT:
	  do {
		  // Return a random value that can be 1 - 8
		  a0.val = static_cast<dirVal_t>((rand() % 8) + 1);
	  } while (((a0.val == UP) && (s0->y == 0)) ||
		  ((a0.val == DOWN) && (s0->y == ySize-1)) ||
		  ((a0.val == RIGHT) && (s0->x == xSize-1)) ||
		  ((a0.val == LEFT) && (s0->x == 0)) ||
		  ((a0.val == UP_RIGHT) && (s0->x == xSize - 1) && (s0->y == 0)) ||
		  ((a0.val == UP_LEFT) && (s0->x == 0) && (s0->y == 0)) ||
		  ((a0.val == DOWN_RIGHT) && (s0->x == xSize - 1) && (s0->y == ySize - 1)) ||
		  ((a0.val == DOWN_LEFT) && (s0->x == 0) && (s0->y == ySize - 1)));
	  break;  
	  // Check for barrier.
  case CHECK_BARRIER:
	  do {
		  ...
		  ((a0.val == UP_RIGHT) && (agentBrain[s0->y-1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x+1].locSt == BARRIER)) ||
		  ((a0.val == UP_LEFT) && (agentBrain[s0->y-1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x-1].locSt == BARRIER)) ||
		  ((a0.val == DOWN_RIGHT) && (agentBrain[s0->y+1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x+1].locSt == BARRIER)) ||
		  ((a0.val == DOWN_LEFT) && (agentBrain[s0->y+1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x-1].locSt == BARRIER)));
	  break;
  ...
  case CHECK_BOTH:
	  do {
		  ...
		  a0.val = static_cast<dirVal_t>((rand() % 8) + 1);
			  ...
			  ((a0.val == UP_RIGHT) && (((s0->x == xSize - 1) && (s0->y == 0)) || ((agentBrain[s0->y-1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x+1].locSt == BARRIER)))) ||
			  ((a0.val == UP_LEFT) && (((s0->x == 0) && (s0->y == 0)) || ((agentBrain[s0->y-1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x-1].locSt == BARRIER)))) ||
			  ((a0.val == DOWN_RIGHT) && (((s0->x == xSize - 1) && (s0->y == ySize - 1)) || ((agentBrain[s0->y+1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x+1].locSt == BARRIER)))) ||
			  ((a0.val == DOWN_LEFT) && (((s0->x == 0) && (s0->y == ySize - 1)) || ((agentBrain[s0->y+1][s0->x].locSt == BARRIER) && (agentBrain[s0->y][s0->x-1].locSt == BARRIER)))));
	  break;
	  // No check.
  case NO_CHECK:
	  // Return a random value that can be 1, 2, 3, or 4.
  ...
  default:
	...
	error_t MLAgent::TakeStep(agent_t *agent, state_t **agentBrain, agent_t* agentPath, int x, int y, int xSize, int ySize, state_t* startLoc, action_t direction, FILE* agentF, learnMethod_t lrnMethod)
	{
		int dummy = 0;
		point_t pt;

		int prevX = agent->loc.x;
		int prevY = agent->loc.y;

		// Check for maze limit first.
		if (this->maze->IsPositionWithinLocalLimits(Framework::Position<int>(x, y)))
		{
			agent->iter = agent->iter + 1;          // This is a new iteration.
			// Check for barrier and move agent up if there is no barrier there.
			if (this->maze->CanMove(this->id, Position<int>(x, y)))
			{
				// Add to list.
				if (lrnMethod == PHS_DISTG_NVSTED_LP)
				{
					agentBrain[agent->loc.y][agent->loc.x].toX = x;
					agentBrain[agent->loc.y][agent->loc.x].toY = y;   
				...
				{
					bool_t actionTaken = FALSE_t;

					switch (direction.val)
					{
					case UP:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.up;
						break;
					case DOWN:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.down;
						break;
					case LEFT:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.left;
						break;
					case RIGHT:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.right;
						break;
					case UP_RIGHT:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.up_right;
						break;
					case UP_LEFT:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.up_left;
						break;
					case DOWN_RIGHT:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.down_right;
						break;
					case DOWN_LEFT:
						actionTaken = agentBrain[agent->loc.y][agent->loc.x].actTaken.down_left;
						break;
					}

					if (actionTaken == TRUE_t)
					{
						if (!Validator::IsNull(log, NAME("log")))
						{
				...
				switch (direction.val)
					...
					case RIGHT:
						...
						break;
					case UP_RIGHT:
						agentBrain[agent->loc.y][agent->loc.x].actTaken.up_right = TRUE_t;
						agentBrain[agent->loc.y][agent->loc.x].nVsted.up_right++;
						break;
					case UP_LEFT:
						agentBrain[agent->loc.y][agent->loc.x].actTaken.up_left = TRUE_t;
						agentBrain[agent->loc.y][agent->loc.x].nVsted.up_left++;
						break;
					case DOWN_RIGHT:
						agentBrain[agent->loc.y][agent->loc.x].actTaken.down_right = TRUE_t;
						agentBrain[agent->loc.y][agent->loc.x].nVsted.down_right++;
						break;
					case DOWN_LEFT:
						agentBrain[agent->loc.y][agent->loc.x].actTaken.down_left = TRUE_t;
						agentBrain[agent->loc.y][agent->loc.x].nVsted.down_left++;
						break;
					}
				...
				this->totalNumberOfStepsTaken++;
				             
				agent->loc.y = y;      // Move the agent.
				agent->loc.x = x;	   // Move the agent.

				agentBrain[agent->loc.y][agent->loc.x].fromX = prevX;
				agentBrain[agent->loc.y][agent->loc.x].fromY = prevY;
				agent->loc.fromX = prevX;
				agent->loc.fromY = prevY;
				agentBrain[agent->loc.y][agent->loc.x].isVsted = TRUE_t;
				agentBrain[agent->loc.y][agent->loc.x].totalNVsted++;
				agentPath[agent->step].dir = direction;
				...
				this->lastDir = direction.val;
			}
			// Record barrier.
			else if (static_cast<locState_t>(this->maze->GetObjectOn(Position<int>(x, y))) == BARRIER)
			{
				Position<int> pos(x, y);

				if (agentBrain[y][x].locSt != BARRIER)
				{
					this->obstaclePosList.push_back(new Position<int>(pos));

					MessageDistributor::Instance()->Broadcast(id, new MazePositionPackage(id, 0, static_cast<char>(BARRIER), pos));
					agentBrain[y][x].locSt = static_cast<locState_t>(this->maze->GetObjectOn(Position<int>(x, y)));    
					agentBrain[y][x].isVsted = NEVER;        
					agent->moved = FALSE_t;
					...
					case UP:
						...
						(agent->loc.x >= 1) ?
						(agentBrain[agent->loc.y][agent->loc.x-1].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x < xSize-1) ?
							(agentBrain[agent->loc.y][agent->loc.x+1].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 2) && (agent->loc.x >= 1)) ?
							(agentBrain[agent->loc.y-2][agent->loc.x-1].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 2) && (agent->loc.x < xSize-1)) ?
							(agentBrain[agent->loc.y-2][agent->loc.x+1].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);
						break;
					case DOWN:
						...
						((agent->loc.x >= 1) && (agent->loc.y < ySize-2)) ?
						(agentBrain[agent->loc.y+2][agent->loc.x-1].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.x < xSize-1) && (agent->loc.y < ySize-2)) ?
							(agentBrain[agent->loc.y+2][agent->loc.x+1].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x >= 1) ?
							(agentBrain[agent->loc.y][agent->loc.x-1].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x < xSize-1) ?
							(agentBrain[agent->loc.y][agent->loc.x+1].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);
						break;
					case LEFT:
						...
						((agent->loc.x >= 2) && (agent->loc.y < ySize-1)) ?
						(agentBrain[agent->loc.y+1][agent->loc.x-2].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y < ySize-1) ?
							(agentBrain[agent->loc.y+1][agent->loc.x].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.x >= 2) && (agent->loc.y >= 1)) ?
							(agentBrain[agent->loc.y-1][agent->loc.x-2].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y >= 1) ?
							(agentBrain[agent->loc.y-1][agent->loc.x].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);
						break;
					case RIGHT:
						...
						(agent->loc.y < ySize-1) ?
						(agentBrain[agent->loc.y+1][agent->loc.x].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y < ySize-1) && (agent->loc.x < xSize-2)) ?
							(agentBrain[agent->loc.y+1][agent->loc.x+2].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y >= 1) ?
							(agentBrain[agent->loc.y-1][agent->loc.x].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 1) && (agent->loc.x < xSize-2)) ?
							(agentBrain[agent->loc.y-1][agent->loc.x+2].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);
						break;
					case UP_RIGHT:
						agentPath[agent->step].loc.barDetect.up_right = TRUE_t;
						// Shadow barrier.      
						agentBrain[agent->loc.y][agent->loc.x].dsRwrd.up_right = -1 * RWRD_FACTOR;      
						((agent->loc.y >= 1) && (agent->loc.x < xSize-2)) ? 
							(agentBrain[agent->loc.y-1][agent->loc.x+2].dsRwrd.left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x < xSize-1) ?
							(agentBrain[agent->loc.y][agent->loc.x+1].dsRwrd.up = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 2) && (agent->loc.x < xSize-1)) ?
							(agentBrain[agent->loc.y-2][agent->loc.x+1].dsRwrd.down = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y >= 1) ?
							(agentBrain[agent->loc.y-1][agent->loc.x].dsRwrd.right = -1 * RWRD_FACTOR) : (dummy = 1);

						(agent->loc.x < xSize-2) ?
							(agentBrain[agent->loc.y][agent->loc.x+2].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y >= 2) ?
							(agentBrain[agent->loc.y-2][agent->loc.x].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 2) && (agent->loc.x < xSize-2)) ?
							(agentBrain[agent->loc.y-2][agent->loc.x+2].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);	
						break;
					case UP_LEFT:
						agentPath[agent->step].loc.barDetect.up_left = TRUE_t;
						// Shadow barrier.      
						agentBrain[agent->loc.y][agent->loc.x].dsRwrd.up_left = -1 * RWRD_FACTOR;      
						(agent->loc.y >= 1) ? 
							(agentBrain[agent->loc.y-1][agent->loc.x].dsRwrd.left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x >= 1) ?
							(agentBrain[agent->loc.y][agent->loc.x-1].dsRwrd.up = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 2) && (agent->loc.x >= 1)) ?
							(agentBrain[agent->loc.y-2][agent->loc.x-1].dsRwrd.down = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 1) && (agent->loc.x >= 2)) ?
							(agentBrain[agent->loc.y-1][agent->loc.x-2].dsRwrd.right = -1 * RWRD_FACTOR) : (dummy = 1);

						(agent->loc.x >= 2) ?
							(agentBrain[agent->loc.y][agent->loc.x-2].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y >= 2) && (agent->loc.x >= 2)) ?
							(agentBrain[agent->loc.y-2][agent->loc.x-2].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y >= 2) ?
							(agentBrain[agent->loc.y-2][agent->loc.x].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);
						break;
					case DOWN_RIGHT:
						agentPath[agent->step].loc.barDetect.down_right = TRUE_t;
						// Shadow barrier.      
						agentBrain[agent->loc.y][agent->loc.x].dsRwrd.down_right = -1 * RWRD_FACTOR;      
						((agent->loc.y < ySize - 1) && (agent->loc.x < xSize-2)) ? 
							(agentBrain[agent->loc.y+1][agent->loc.x+2].dsRwrd.left = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y < ySize - 2) && (agent->loc.x < xSize-1)) ?
							(agentBrain[agent->loc.y+2][agent->loc.x+1].dsRwrd.up = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x < xSize-1) ?
							(agentBrain[agent->loc.y][agent->loc.x+1].dsRwrd.down = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y < ySize - 1) ?
							(agentBrain[agent->loc.y+1][agent->loc.x].dsRwrd.right = -1 * RWRD_FACTOR) : (dummy = 1);

						((agent->loc.y < ySize - 2) && (agent->loc.x < xSize-2)) ?
							(agentBrain[agent->loc.y+2][agent->loc.x].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y < ySize - 2) ?
							(agentBrain[agent->loc.y+2][agent->loc.x+2].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x < xSize-2) ?
							(agentBrain[agent->loc.y][agent->loc.x+2].dsRwrd.down_left = -1 * RWRD_FACTOR) : (dummy = 1);
						break;
					case DOWN_LEFT:
						agentPath[agent->step].loc.barDetect.down_left = TRUE_t;
						// Shadow barrier.      
						agentBrain[agent->loc.y][agent->loc.x].dsRwrd.down_left = -1 * RWRD_FACTOR;      
						((agent->loc.y < ySize - 1) && (agent->loc.x < xSize-2)) ? 
							(agentBrain[agent->loc.y+1][agent->loc.x].dsRwrd.left = -1 * RWRD_FACTOR) : (dummy = 1);
						((agent->loc.y < ySize - 2) && (agent->loc.x < xSize-1)) ?
							(agentBrain[agent->loc.y+2][agent->loc.x-1].dsRwrd.up = -1 * RWRD_FACTOR) : (dummy = 1);
						...
						((agent->loc.y < ySize - 2) && (agent->loc.x < xSize-2)) ?
							(agentBrain[agent->loc.y+2][agent->loc.x-2].dsRwrd.up_right = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.y < ySize - 2) ?
							(agentBrain[agent->loc.y+2][agent->loc.x].dsRwrd.up_left = -1 * RWRD_FACTOR) : (dummy = 1);
						(agent->loc.x < xSize-2) ?
							(agentBrain[agent->loc.y][agent->loc.x-2].dsRwrd.down_right = -1 * RWRD_FACTOR) : (dummy = 1);	
						break;
					}					
				}
			}
		}
		else if (this->maze->IsPositionWithinGlobalLimits(Framework::Position<int>(x, y)))
		{
			unsigned node;
			char newStart;
			...
			{
				if (this->maze->CanMoveToGlobal(this->id, Framework::Position<int>(agent->loc.x, agent->loc.y), Framework::Position<int>(x, y), node, newStart))
				{
					this->lastDir = direction.val;
					...
					// Set agent's prev pos before moving to the other node.
					this->prevTransferedPos.SetXCoor(agent->loc.x);
					this->prevTransferedPos.SetYCoor(agent->loc.y);
				...
				else if (static_cast<locState_t>(this->maze->GetObjectOn(Position<int>(x, y))) == BARRIER)
				{
					...
					{
						if (((*iter)->GetXCoor() == x) && ((*iter)->GetYCoor() == y))
						{
							isInList = true;
							break;
						}
					...
					if (!isInList)
					{
						this->obstaclePosList.push_back(new Position<int>(x, y));
					}
				}
			}
		}
		...
		learnMethod_t lrnMethod, state_t *startLoc, FILE *agentF)
	{
		...
		Position<float> nextPos;

		switch(a0.val)
		{
		case UP:
			nextPos.SetXCoor(agent->loc.x);
			nextPos.SetYCoor(agent->loc.y - 1);
			break;
		case DOWN:
			nextPos.SetXCoor(agent->loc.x);
			nextPos.SetYCoor(agent->loc.y + 1);
			break;
		case RIGHT:
			nextPos.SetXCoor(agent->loc.x - 1);
			nextPos.SetYCoor(agent->loc.y);
			break;
		case LEFT:
			nextPos.SetXCoor(agent->loc.x + 1);
			nextPos.SetYCoor(agent->loc.y);
		...
		Position<float> currPos(agent->loc.x, agent->loc.y);//(1,1);

		/*if (agent->step > 2)
		{
			stay = !this->IsTurningAngleAcceptable(currPos - prevPos, nextPos - currPos);
		}*/
		//nextPos.SetXCoor(0);
		//nextPos.SetYCoor(0);

		/*Framework::Position<float> pos1(0, 0);
		Framework::Position<float> pos2(1, 0);
		Framework::Position<float> pos3(2, 1);

		stay = this->IsRadiusOfCurvatureAcceptable(pos1, pos2, pos3);
		...
		//stay = !this->IsRadiusOfCurvatureAcceptable(prevPos, currPos, nextPos);
		/*this->goalPos.SetXCoor(0);
		this->goalPos.SetYCoor(5);
		Framework::Position<float> pos1(0, 1);
		Framework::Position<float> pos2(1, 1);
		nextPos.SetXCoor(0);
		nextPos.SetYCoor(0);		

		float totalDiscountedReward = agent->loc.dsRwrd.down + agent->loc.dsRwrd.up + agent->loc.dsRwrd.left + agent->loc.dsRwrd.right;

		stay = !this->IsRadiusOfCurvatureAcceptable(pos1, pos2, nextPos, agent->loc.totalNVsted, totalDiscountedReward);*/

		/*if (agent->step >= 2)
		{
			stay = !this->IsRadiusOfCurvatureAcceptable(prevPos, currPos, nextPos, agent->loc.totalNVsted, totalDiscountedReward);
		}*/

		///*this->goalPos.SetXCoor(3);
		//this->goalPos.SetYCoor(2);*/
		//nextPos.SetXCoor(1);
		//nextPos.SetYCoor(0);

		//stay = !this->IsRadiusOfCurvatureAcceptable(pos1, pos2, nextPos, agent->loc.totalNVsted, totalDiscountedReward);

		/////*this->goalPos.SetXCoor(4);
		////this->goalPos.SetYCoor(2);*/
		//nextPos.SetXCoor(2);
		//nextPos.SetYCoor(0);
		...
		// Only if the agent has moved, should this be true.
		agent->moved = FALSE_t;
		...
		if (!Validator::IsNull(agent, NAME("agent")))
		{
			if (!stay)
			{
				switch (a0.val) 
				{
					// Direction is UP.
				case UP:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x, agent->loc.y - 1, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
					// Direction is DOWN.
				case DOWN:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x, agent->loc.y + 1, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					...
					// Direction is RIGHT.
				case RIGHT:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x + 1, agent->loc.y, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
					// Direction is LEFT.
				case LEFT:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x - 1, agent->loc.y, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
					// Direction is UP_RIGHT.
				case UP_RIGHT:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x + 1, agent->loc.y - 1, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
					// Direction is UP_LEFT.
				case UP_LEFT:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x - 1, agent->loc.y - 1, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
					// Direction is DOWN_RIGHT.
				case DOWN_RIGHT:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x + 1, agent->loc.y + 1, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
					// Direction is DOWN_LEFT.
				case DOWN_LEFT:
					err = this->TakeStep(agent, agentBrain, agentPath, agent->loc.x - 1, agent->loc.y + 1, xSize, ySize, startLoc, a0, agentF, lrnMethod);
					break;
				case NO_DIR:
		...
		do
		{
			...
			if (mazePos != NULL)
			{
				...
				if (agentBrain[yCoor][xCoor].locSt != BARRIER)
				{
					this->obstaclePosList.push_back(new Position<int>(mazePos->GetPosition()));
	...
	/////////////////////////////////////////////////////////////////////////////////
	float 
		MLAgent::storeRewards (float gamma, agent_t *agentPath, state_t **agentBrain, int agentBrainSize, int fVerb, FILE *agentF, int numStepsTaken)
	{
		int i;                   // index.
		...
		for (i = agentBrainSize; i >= 1; i--) {
			// Evaluate the discounted reward value for current location.
			dsReward = pow(gamma, numStepsTaken-i) * RWRD_FACTOR;
			dsCmReward += dsReward;
	...
	case UP:
		...
		if (this->isGoalOnThisNode && i == agentBrainSize) {
  ...
  default:
	...
	action_t MLAgent::ConvertDirectionToAction(Framework::Position<float> direction)
	{
		action_t a0;
		a0.type = POLICY;
		a0.val = NO_DIR;
		
		if (direction.GetXCoor() > 0)
		{
			a0.val = RIGHT;

			if (direction.GetYCoor() > 0)
			{
				// positive values go down.  Negative values go up.
				a0.val = DOWN_RIGHT;
			}
			else if (direction.GetYCoor() < -0.5)
			{
				a0.val = UP_RIGHT;
			}
		}
		else if (direction.GetXCoor() < 0)
		{
			a0.val = LEFT;
		...
		/*if (abs(distance.GetXCoor()) >= abs(distance.GetYCoor()))
		{
			if (distance.GetXCoor() < 0)
			{
				a0.val = LEFT;
			}
			else if (distance.GetXCoor() > 0)
			{
				a0.val = RIGHT;
			}
		}
		...
		{
			if (distance.GetYCoor() < 0)
			{
				a0.val = UP;
			}
			else if (distance.GetYCoor() > 0)
			{
				a0.val = DOWN;
			}
		}*/
		...
		if (!Validator::IsNull(log, NAME("log")))
		{
			log->Log(&typeid(MLAgent), "Take Action on gradient descent: [Direction vector: (%f, %f)], [Direction: %s]", direction.GetXCoor(), direction.GetYCoor(), a0.name);
		}

		return a0;
	}

	action_t MLAgent::takeActionOnGradientDescent (Framework::Position<float> goal, Framework::Position<float>& currentPosition, int totalNumVisited, float totalDiscountedReward, Algorithm algorithm, float stepSize)
	{
		Framework::Position<float> oldPosition = currentPosition;
		Framework::Position<float> rawStep = (*algorithm)(goal, currentPosition, totalNumVisited, totalDiscountedReward, this->obstaclePosList, this->info.absalonSquared) * stepSize;
		Framework::Position<float> step;
		action_t action = MLAgent::CalculateActionAndStep(rawStep, step);

		currentPosition = oldPosition - step;		
  ...
  default:
	  break;
			...
			if ((taw != GREEDY_TAW) &&
				...
				else if ((maxPas == probDown)&&(s0->y < ySize - 1)) { 
					a0.val = DOWN;
					a0.type = POLICY;
				}
				else if ((maxPas == probRight)&&(s0->x < xSize - 1)) {
					a0.val = RIGHT;
					a0.type = POLICY;
				}
  ...
  default:
	  ...
	  break;
		...
		if ((newSta.x != s.x) || (newSta.y != s.y))
		{
			newSta.fromX = s.x;
			newSta.fromY = s.y;

			if ((newSta.x < xSize) && (newSta.y < ySize) && (newSta.x >= 0) && (newSta.y >= 0))
			{
				newSta.totalNVsted = agentBrain[newSta.y][newSta.x].totalNVsted + 1;
				newSta.dsRwrd.up = agentBrain[newSta.y][newSta.x].dsRwrd.up;
				newSta.dsRwrd.left = agentBrain[newSta.y][newSta.x].dsRwrd.left;
				newSta.dsRwrd.right = agentBrain[newSta.y][newSta.x].dsRwrd.right;
			newSta.dsRwrd.down = agentBrain[newSta.y][newSta.x].dsRwrd.down;
			}
		}
