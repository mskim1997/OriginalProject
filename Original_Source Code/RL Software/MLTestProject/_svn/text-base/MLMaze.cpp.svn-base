...
namespace MLTest
{
	...
	void MLMaze::NotifyAgentIsDone(unsigned agentId, bool isTransferedAgent, bool hasFoundGoal)
	{
		sem_wait(&this->binSem);
		...
		if (this->agentDonePublisher != NULL)
		{
			this->agentDonePublisher->Publish(new AgentDoneEventPackage(this->nodeId, 0, agentId, 0, 0, 0, isTransferedAgent, hasFoundGoal));
		}
